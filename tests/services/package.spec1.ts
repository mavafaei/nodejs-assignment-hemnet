import { Municipality } from '../../models/municipality';
import { MunicipalityPackages } from '../../models/municipalityPackages';
import { Package } from '../../models/package';
import PackageService from '../../services/package.service';
import { Price } from '../../models/price'
import { sequelizeConnection } from '../../db/config';

describe('PackageService', () => {
  // Set the db object to a variable which can be accessed throughout the whole test file
  const db = sequelizeConnection;
  const packageService = PackageService;

  // Before any tests run, clear the DB and run migrations with Sequelize sync()
  beforeEach(async () => {
    await db.sync({ force: true });
  });

  afterAll(async () => {
    await db.close();
  });

  it('Updates the current price of the provided package', async () => {
    const pack = await Package.create({ name: 'Dunderhonung' });

    const newPackage = await packageService.updatePackagePrice({ packageId: pack.id, newPriceCents: 200_00 });

    expect(newPackage).not.toBeNull();
    expect(newPackage!.priceCents).toBe(200_00);
  });

  it('Stores the old price of the provided package in its price history', async () => {
    const pack = await Package.create({ name: 'Dunderhonung' });

    // set first price to 100_00
    await packageService.updatePackagePrice({ packageId: pack.id, newPriceCents: 100_00 });

    // update price to 200_00
    await packageService.updatePackagePrice({ packageId: pack.id, newPriceCents: 200_00 });

    const municipalityPackage = await MunicipalityPackages.findOne({ where: { packageId: pack.id, municipalityId: null } });
    const priceHistory = await Price.findAll({ where: { municipalityPackageId: municipalityPackage!.id } });


    expect(municipalityPackage).not.toBeNull();
    expect(priceHistory.length).toBe(1);
    expect(priceHistory[0].priceCents).toBe(100_00);
  });

  // // This tests cover feature request 1. Feel free to add more tests or change
  // // the existing one.
  it('Supports adding a price for a specific municipality', async () => {
    const pack = await Package.create({ name: 'Dunderhonung' });
    const municipality = await Municipality.create({ name: 'Göteborg' });
    // set first price to 100_00 for Gothenburg
    await packageService.updatePackagePrice({ packageId: pack.id, newPriceCents: 100_00, municipalityId: municipality.id });

    // set first price to 200_00 for Gothenburg
    await packageService.updatePackagePrice({ packageId: pack.id, newPriceCents: 200_00, municipalityId: municipality.id });

    const response = await packageService.priceFor({ packageId: pack.id, municipalityId: municipality.id });

    expect(response).toBe(200_00);
  });
});
