...
describe('PriceService', () => {
  ...
  it('Returns the pricing history for the provided year and package', async () => {
    ...
    // I was getting `serializes to the same string` error, so I just changed toBe to toEqual
    expect(await PriceService.getPriceHistory({ packageId: basic.id, date })).toEqual({
      Göteborg: [30_00],
  ...
  it('Supports filtering on municipality', async () => {
    ...
    expect(await PriceService.getPriceHistory({ packageId: basic.id, municipalityId: municipalityGoteborg.id, date })).toEqual({
      Göteborg: [20_00, 60_00, 30_00, 70_00],
    });


    expect(await PriceService.getPriceHistory({ packageId: basic.id, municipalityId: municipalityStockholm.id, date })).toEqual({
